...
if web:
    ...
    def show_entries():
        general_Data = {
        'title' : 'Garyware v0.24'}
        stream_entries = return_dict(DATABASE)
        return render_template('main4a_bootstrap.html', entries = stream_entries, **general_Data)
    ...
    # Shutdown the computer now on navbar - move to modal
    @app.route('/shutdown')
    def shutdown_now():
    ...
    def station():
        ...
        run_cmd('mpc current')
        while True:
          station = os.popen('mpc current').readline()
          time.sleep(15)
        print (station)
        return render_template('main4a_bootstrap.html', station=station, **general_Data)
    ...
    def current_artist():
        atitle = os.popen('mpc -f %artist%').readline()
        return(atitle.strip())
